...
const ChatPage = () => {
  ...
  const handleSendMessage = async (text, isImageAnalysis = false) => {
    ...
    try {
      const botReply = await sendMessageToAI(text);
      const isObjectReply = botReply !== null && typeof botReply === 'object';

      const botMsg = {
        id: (Date.now() + 1).toString(),
        text: isObjectReply ? botReply : { reply: botReply },
        type: isObjectReply ? 'structured' : 'text',
        sender: 'bot',
        timestamp: new Date().toISOString()
      };
